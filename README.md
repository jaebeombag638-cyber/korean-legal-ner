...
- **span 위치 불일치** (전체 341,355건 중 13.09%): `section_text[start:end]`가 `original_text`와 어긋나는 annotation이 존재. 원인은 (1) 원문이 이미 `#이름#` 형태 placeholder로 치환된 경우(5.23%), (2) annotation 계산 시점과 배포된 텍스트 버전이 달라 위치가 밀린 경우(7.86%) 두 가지로 추정. 학습 시 불일치 annotation은 필터링하여 제외
